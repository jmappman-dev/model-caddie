...
- **`provider`** (string, required): a label naming who serves this lane. It shows up in dispatch strings and in the review pass's descriptions of who reviewed what. It is not checked against a fixed list of providers, but it is restricted to a plain character set (letters, digits, `. : / @ + - _`, starting with a letter or digit, max 80 characters) because it reaches stdout and the calling agent verbatim; spaces, quotes, and shell metacharacters are rejected.
  ...
  **The review pass compares providers by vendor root, so choose labels deliberately.** Two lanes are refused as a reviewer pair when their labels share a root, taken as everything before the first `-`, `_`, `/`, `:`, `.` or space, lowercased. That is what stops `openai` and `openai-codex`, or `openai` and `openai_codex`, from reviewing each other: two models from one vendor share training data, tooling and blind spots, so they are not an independent pass.

  The rule is deliberately blunt and it over-matches in one direction. Labels like `api.openai.com` and `api.anthropic.com` both reduce to `api`, and URL-style labels both reduce to `https`, so they are treated as the same vendor and the reviewer is refused. The consequence is never a false claim of independence: the requirement stays and the run asks for a manual second pass instead. If you use host-style or URL-style labels, put the vendor first (`openai-api`, not `api.openai.com`) so the root carries meaning.
